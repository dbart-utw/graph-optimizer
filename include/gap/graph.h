...
class CSRGraph {
 ...
 public:
  ...
  DestID_** out_index() const {
    return out_index_;
  }

  DestID_* out_neighs() const {
  ...
  void PrintCSR() const {
    ...
    for (NodeID_ i=0; i < num_nodes_+1; i++)
      std::cout << out_index_[i] << "(" << *(int*)out_index_[i] << ") ";
    std::cout << std::endl;
    ...
    if (directed_) {
      std::cout << "In Index: ";
      for (NodeID_ i=0; i < num_nodes_+1; i++)
        std::cout << in_index_[i] << "(" << *(int*)in_index_[i] << ") ";
      std::cout << std::endl;
      std::cout << "In Neighbors: ";
      for (int64_t i=0; i < num_edges_; i++)
        std::cout << in_neighbors_[i] << "(" << &in_neighbors_[i] << ") ";
      std::cout << std::endl;
    }
  }
  ...
  EdgeListStruct MakeEdgeListStruct() {
    EdgeListStruct els;
    els.num_nodes = num_nodes_;
    els.num_edges = num_edges_;
    els.degrees = new int32_t[num_nodes_];
    els.sources = new int32_t[num_edges_];
    els.destinations = new int32_t[num_edges_];
    int edge_idx = 0;
    for (NodeID_ n=0; n < num_nodes_; n++) {
      els.degrees[n] = out_degree(n);
      for (DestID_ j : out_neigh(n)) {
        els.sources[edge_idx] = n;
        els.destinations[edge_idx++] = j;
      }
    }
    return els;
  }

  EdgeStructList MakeEdgeStructList() {
    EdgeStructList esl;
    esl.num_nodes = num_nodes_;
    esl.num_edges = num_edges_;
    esl.degrees = new int32_t[num_nodes_];
    esl.edges = new EdgeStruct[num_edges_];
    int edge_idx = 0;
    for (NodeID_ n=0; n < num_nodes_; n++) {
      esl.degrees[n] = out_degree(n);
      for (DestID_ j : out_neigh(n)) {
        esl.edges[edge_idx].u = n;
        esl.edges[edge_idx++].v = j;
      }
    }
    return esl;
  }
