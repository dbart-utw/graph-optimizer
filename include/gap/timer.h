...
class Timer {
 public:
  ...
  static void PrintTime(double seconds, const char *label) {
    printf("%s: %.3f s\n", label, seconds);
  }

  static void PrintTimeMS(double ms, const char *label) {
    printf("%s: %.3f ms\n", label, ms);
  }

  static void PrintTimeUS(double us, const char *label) {
    printf("%s: %.3f us\n", label, us);
  }
